...
class SpectrogramPreprocessor():
    ...
    def __init__(self, ext='.wav', sample_rate=16000, window_size=0.02, stride=0.01, power_spectrum=True, num_mels=40, 
    ...
    def __call__(self, path, trans_dict):
        """
        ...
        """
        pcm, sample_rate = librosa.load(path, sr=self.sample_rate)
...
class TextPreprocessor():
    ...
    def __call__(self, path, trans_dict):
        ...
        #    path = path + self.ext
        trans = trans_dict[os.path.splitext(os.path.basename(path))[0]]

        #with open(path, 'r') as text_file:
        transcript_raw = self.clean_func(trans)
